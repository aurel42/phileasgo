...
func (m *TestPOIProvider) GetNarrationCandidates(limit int, minScore *float64, isOnGround bool) []*model.POI {
	return []*model.POI{}
}
...
func TestSampleNarrationLength_RelativeDominance(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		val, _ := svcHigh.sampleNarrationLength(&model.POI{Score: 100}, "", 1000)
		sumHigh += val
	}
	...
	sumLow := 0
	for i := 0; i < iterations; i++ {
		val, _ := svcLow.sampleNarrationLength(&model.POI{Score: 100}, "", 1000)
		sumLow += val
	}
